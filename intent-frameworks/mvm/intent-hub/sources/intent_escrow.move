...
/// allowing users to create escrows with simple yes/no approver approval.
///
/// **IMPORTANT**: This module uses "approver" terminology in its public API,
/// but internally calls oracle functions from fa_intent_with_oracle.move.
/// The approver IS an oracle - we use oracle implementation to create approver functionality.
///
...
module mvmt_intent::intent_escrow {
    ...
    /// Creates a simple escrow with approver approval requirement
    ///
    /// # Arguments
    ...
    public fun create_escrow(
        ...
        // Create the approver-guarded intent with placeholder values
        ...
        let placeholder_amount = 0; // Placeholder (escrow validation is done off-chain by approver)
        ...
        fa_intent_with_oracle::create_fa_to_fa_intent_with_oracle_requirement(
            ...
            option::none(), // Escrows don't validate desired_metadata_addr - validation happens on hub chain
            expiry_time,
            signer::address_of(requester_signer),
            requirement,
            false, // CRITICAL: escrow intents MUST be non-revocable for security!
            //      This ensures funds can ONLY be released by approver approval/rejection
            //      Approvers can safely trigger actions elsewhere based on deposit events
            intent_id,
    ...
    /// Completes an escrow with approver approval
    ///
    /// The approver signs the intent_id - the signature itself is the approval.
    ...
    /// - `approver_signature`: Approver's Ed25519 signature (signs the intent_id)
    ///
    /// # Aborts
    /// - If approver signature verification fails (wrong intent_id or invalid signature)
    ...
    public fun complete_escrow(
        ...
        approver_signature: ed25519::Signature
    ) {
        ...
        // Create approver witness - signature itself is the approval, reported_value is just metadata
