...
# Verify Integrated GMP EVM Address Script (Testnet)
#
# Verifies that INTEGRATED_GMP_EVM_PUBKEY_HASH in this directory's .env.testnet matches
# the EVM address derived from INTEGRATED_GMP_PRIVATE_KEY (and INTEGRATED_GMP_PUBLIC_KEY), and optionally
# checks that the on-chain IntentEscrow contract has the correct approver address.
#
# Usage: ./verify-integrated-gmp-evm-address.sh
#
# Checks:
#   1. Computes EVM address from INTEGRATED_GMP_PRIVATE_KEY (via get_approver_eth_address)
#   2. Compares to INTEGRATED_GMP_EVM_PUBKEY_HASH in this directory's .env.testnet
...
# Check if INTEGRATED_GMP_EVM_PUBKEY_HASH is set
if [ -z "$INTEGRATED_GMP_EVM_PUBKEY_HASH" ]; then
    echo "❌ ERROR: INTEGRATED_GMP_EVM_PUBKEY_HASH not set in $ENV_FILE"
    exit 1
fi

# Check if INTEGRATED_GMP_PRIVATE_KEY is set
if [ -z "$INTEGRATED_GMP_PRIVATE_KEY" ]; then
    echo "❌ ERROR: INTEGRATED_GMP_PRIVATE_KEY not set in $ENV_FILE"
    exit 1
fi

echo " Verifying Integrated GMP EVM Address"
echo "======================================="
echo ""
echo "   Config file: $ENV_FILE"
echo "   Expected:    $INTEGRATED_GMP_EVM_PUBKEY_HASH"
echo ""

# Compute EVM address from private key (get_approver_eth_address reads INTEGRATED_GMP_* from env when building config)
echo "   Computing EVM address from INTEGRATED_GMP_PRIVATE_KEY..."
COMPUTED_ADDR=$(cd "$PROJECT_ROOT/integrated-gmp" && \
...
if [ -z "$COMPUTED_ADDR" ]; then
    ...
    echo "   Make sure INTEGRATED_GMP_PRIVATE_KEY and INTEGRATED_GMP_PUBLIC_KEY are set in $ENV_FILE (and sourced by integrated-gmp config)"
    exit 1
fi
...
EXPECTED_NORM=$(echo "$INTEGRATED_GMP_EVM_PUBKEY_HASH" | tr '[:upper:]' '[:lower:]')
...
if [ "$EXPECTED_NORM" = "$COMPUTED_NORM" ]; then
    ...
    echo "   Computed: $COMPUTED_ADDR"
    echo ""
    echo "   Action: Update INTEGRATED_GMP_EVM_PUBKEY_HASH in $ENV_FILE to:"
    echo "   INTEGRATED_GMP_EVM_PUBKEY_HASH=$COMPUTED_ADDR"
fi
...
if [ -f "$INTEGRATED_GMP_CONFIG" ]; then
    ...
    ESCROW_ADDR=$(grep -A5 "\[connected_chain_evm\]" "$INTEGRATED_GMP_CONFIG" | grep "escrow_contract_addr" | sed 's/.*= *"\(.*\)".*/\1/' | tr -d '"' | head -1)
    RPC_URL=$(grep -A5 "\[connected_chain_evm\]" "$INTEGRATED_GMP_CONFIG" | grep "rpc_url" | sed 's/.*= *"\(.*\)".*/\1/' | tr -d '"' | head -1)
