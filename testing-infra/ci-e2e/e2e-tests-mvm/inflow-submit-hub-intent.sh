...
setup_logging "inflow-submit-hub-intent-mvm${MVM_INSTANCE}"
...
aptos move run --profile requester-chain1 --assume-yes \
    ...
    --args "address:${OFFERED_METADATA_MVMCON}" "u64:${OFFERED_AMOUNT}" "u64:${CONNECTED_CHAIN_ID}" "address:${DESIRED_METADATA_HUB}" "u64:${DESIRED_AMOUNT}" "u64:${HUB_CHAIN_ID}" "u64:${EXPIRY_TIME}" "address:${INTENT_ID}" "address:${RETRIEVED_SOLVER}" "address:${SOLVER_MVMCON_ADDR}" "hex:${SOLVER_SIGNATURE_HEX}" "address:${REQUESTER_MVMCON_ADDR}" "u64:${FEE_IN_OFFERED_TOKEN}" >> "$LOG_FILE" 2>&1
