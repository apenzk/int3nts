...
# Start Solver Service for E2E Tests (EVM Connected Chains)
#
# This script generates a solver configuration for both EVM connected chain instances
...
log ""
log " Starting Solver Service (EVM Connected Chains)..."
log "========================================"
log_and_echo " All output logged to: $LOG_FILE"
log ""

# Generate solver config for EVM E2E tests (both instances)
generate_solver_config_evm() {
    ...
    cat > "$config_file" << EOF
# Auto-generated solver config for EVM E2E tests (multi-instance)
# Generated at: $(date)
...
EOF
    ...
    for n in 2 3; do
        ...
        if [ -z "$evm_token_addr" ]; then
            log_and_echo "❌ ERROR: USD_EVM_ADDR not found in chain-info-evm${n}.env"
            exit 1
        fi
        local escrow_addr="${ESCROW_GMP_ADDR:-}"
        if [ -z "$escrow_addr" ]; then
            log_and_echo "❌ ERROR: ESCROW_GMP_ADDR not found in chain-info-evm${n}.env"
            exit 1
        fi
        ...
        log "   - EVM instance $n: RPC=$EVM_RPC_URL chain_id=$EVM_CHAIN_ID escrow=$escrow_addr"

        cat >> "$config_file" << EOF

[[connected_chain]]
type = "evm"
name = "EVM Connected Chain $n (E2E Test)"
rpc_url = "$EVM_RPC_URL"
...
# Gas token (ETH) on EVM chain instance $n
[[liquidity.threshold]]
chain_id = $EVM_CHAIN_ID
token = "0x0000000000000000000000000000000000000000"
min_balance = 1

# USDcon on EVM chain instance $n
[[liquidity.threshold]]
chain_id = $EVM_CHAIN_ID
token = "$usdcon_metadata_evm"
min_balance = 1
EOF
    done
