...
get_svm_balance() {
    ...
    SVM_TOKEN_ACCOUNT="$token_account" SVM_RPC_URL="$SVM_RPC_URL" \
...
get_svm_sol_balance() {
    ...
    svm_cmd "solana balance \"$pubkey\" --url \"$SVM_RPC_URL\"" \
        | awk '{print $1}' | tail -n 1
}

if [ -z "$SOLVER_SVM_TOKEN_ACCOUNT" ] || [ -z "$REQUESTER_SVM_TOKEN_ACCOUNT" ]; then
    log_and_echo "❌ ERROR: SVM token accounts not found in chain-info-svm${SVM_INSTANCE}.env"
...
log_and_echo ""
log_and_echo "   Connected SVM (instance $SVM_INSTANCE, chain ID $SVM_CHAIN_ID):"
