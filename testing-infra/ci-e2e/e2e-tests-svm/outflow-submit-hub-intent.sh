...
setup_logging "submit-hub-intent-svm-outflow${SVM_INSTANCE}"
...
log "   Offered amount: $OFFERED_AMOUNT (1 USDhub on hub)"
log "   Desired amount: $DESIRED_AMOUNT (0.985 USDcon on connected SVM, fee-adjusted)"
...
DESIRED_METADATA_SVM="$SVM_TOKEN_HEX"

log "     Outflow configuration:"
log "       Offered metadata (hub):           $OFFERED_METADATA_HUB"
log "       Desired metadata (connected SVM): $DESIRED_METADATA_SVM"
...
aptos move run --profile requester-chain1 --assume-yes \
    ...
    --args "address:${OFFERED_METADATA_HUB}" "u64:${OFFERED_AMOUNT}" "u64:${HUB_CHAIN_ID}" "address:${DESIRED_METADATA_SVM}" "u64:${DESIRED_AMOUNT}" "u64:${CONNECTED_CHAIN_ID}" "u64:${EXPIRY_TIME}" "address:${INTENT_ID}" "address:${REQUESTER_SVM_ADDR}" "address:${RETRIEVED_SOLVER}" "address:${SOLVER_SVM_ADDR}" "hex:${SOLVER_SIGNATURE_HEX}" "u64:${FEE_IN_OFFERED_TOKEN}" >> "$LOG_FILE" 2>&1
